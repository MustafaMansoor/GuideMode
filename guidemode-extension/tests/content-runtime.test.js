...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('[data-guidemode]').count(),0);assert.equal(await page.locator('#guidemode-next-step').count(),0);
    ...
    await page.evaluate(plan=>globalThis.__GuideModeTest.applyVisualPlan(plan),{elements:[],uncertain_refs:[],guide_state:{goal:'Find service',status:'guiding',instruction:'Use Search',target:{ref:search.ref,name:'Search'},supportingRefs:[submit.ref]}});
    assert.equal(await page.locator('#search').getAttribute('data-guidemode'),'current');assert.equal(await page.locator('#guidemode-next-step').textContent(),'Next stepUse Search');assert.equal(await page.locator('#submit').getAttribute('data-guidemode'),'supporting');
    ...
    await page.evaluate(() => globalThis.__GuideModeTest.setVisualMode(true));
    assert.equal(await page.locator('#search').getAttribute('data-guidemode'), 'current'); assert.equal(await page.locator('#small').isChecked(), stateBefore);
    await page.evaluate(()=>globalThis.__GuideModeTest.setVisualMode(false));assert.equal(await page.locator('#search').getAttribute('data-guidemode'),null);
    await page.evaluate(plan=>{globalThis.__GuideModeTest.setVisualMode(true);globalThis.__GuideModeTest.applyVisualPlan(plan)},{elements:[],uncertain_refs:[],guide_state:{goal:'A different goal',status:'guiding',instruction:'Review submit',target:{ref:submit.ref,name:'Submit application'},supportingRefs:[]}});assert.equal(await page.locator('#search').getAttribute('data-guidemode'),null);assert.equal(await page.locator('#submit').getAttribute('data-guidemode'),'current');
