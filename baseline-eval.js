...
const MAX_STEPS = 10;
const PROMPT_VERSION = 'simple-baseline-v1';
...
// Intentionally basic native Playwright execution: no verification, fallback,
// stale-ref recovery, or verification-driven replanning.
async function execute(page, action) {
  ...
  if (action.action === 'check') await locator.check({ timeout: 3000 });
  if (action.action === 'uncheck') await locator.uncheck({ timeout: 3000 });
...
(async () => {
  ...
  const report = { evaluation_id: crypto.randomUUID(), architecture: 'simple-baseline-v1', model: MODEL,
  ...
  const file = path.join(directory, `baseline-evaluation-${new Date().toISOString().replace(/[:.]/g, '-')}.json`);
